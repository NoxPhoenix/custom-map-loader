...
const directoryContainsMapInfo = (contents = []) => {
  if (contents.length < 3) return false;
  const containsImage = contents.find((name) => (name.includes('.jpg') || name.includes('.png')));
  const containsMap = contents.find((name) => (name.includes('.upk') || name.includes('.udk')));
  const containsInfo = contents.includes('info.json');
  if (containsImage && containsInfo && containsMap) return true;
  return false;
};

const getRealDirectory = (directory) => {
  const results = fs.readdirSync(directory);
  if (directoryContainsMapInfo(results)) return directory;
  if (directory.length === 1) return getRealDirectory(path.join(directory, results[0]));
...
const importMap = async (importedMapPath, customMapDirectory) => {
  ...
  const realDirectory = getRealDirectory(stagedContentDirectory);
  const mapFiles = fs.readdirSync(realDirectory);
