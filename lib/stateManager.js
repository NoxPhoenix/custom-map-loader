...
const sendUpdateCardsMessage = _.debounce(() => state.mainWindow.webContents.send('updateCards'), 1000);

const registerListener = (watcher) => {
  watcher.on('change', sendUpdateCardsMessage);
};
...
if (state.customMapDirectory) app.whenReady().then(() => updateDirectoryWatcher(state.customMapDirectory));
...
// ipcMain.on('logDirs', () => dialog.showMessageBox({ title: 'dirs', message: 'Info', detail: `${CLine}` }));
// ipcMain.on('logFile', () => fs.writeFileSync('C:\\Users\\mrmik\\Documents\\Maps\\log.txt', JSON.stringify(CLine)));
